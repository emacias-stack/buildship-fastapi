...
class TestAPIKeyMiddleware:
    ...
    def test_api_key_disabled(self):
        app = FastAPI()
        app.add_middleware(
            APIKeyMiddleware, settings=DummySettings(enable_api_key_auth=False)
        )
    ...
    def test_api_key_valid(self):
        app = FastAPI()
        app.add_middleware(
            APIKeyMiddleware, settings=DummySettings(api_keys=["valid-key"])
        )
    ...
    def test_api_key_skip_exclude_path(self):
        app = FastAPI()
        app.add_middleware(
            APIKeyMiddleware, settings=DummySettings(exclude_api_key_paths=["/docs"])
        )
